                ...
                element_s *gen, uint8_t *recv_id, size_t recv_id_len, pairing_s *pairing)
{
  ...
  // Symmetric key_state encryption setup
	uint8_t *ibe_encrypted_symm_key_ptr = out + rP_length;
	uint8_t *chacha_nonce_ptr = ibe_encrypted_symm_key_ptr + crypto_ghash_BYTES;
	uint8_t *chacha_ciphertext_ptr = chacha_nonce_ptr + crypto_NBYTES;
