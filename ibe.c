...
int ibe_encrypt(byte_t *out, byte_t *msg, uint32_t msg_len, element_s *public_key,
  ...
  //printhex("hash for generating public key_state for chris", id_hash, crypto_ghash_BYTES);
  ...
  printf("%p -> ", &ibe_encrypted_symm_key_ptr); printhex("encrypted symmetric key_state", ibe_encrypted_symm_key_ptr, crypto_ghash_BYTES);
...
int ibe_decrypt(byte_t *out, byte_t *c, uint32_t clen, element_s *private_key, pairing_s *pairing) {
  ...
  byte_t *chacha_nonce = enc_symm_key + crypto_ghash_BYTES;
  ...
  printf("%p -> ", &enc_symm_key); printhex("dec encrypted symmetric key_state", enc_symm_key, crypto_ghash_BYTES);
 ...
 */ element_t u, prg;
  ...
  //printhex("Secret key_state after decryption", secret_key, crypto_ghash_BYTES);
  int res;
  //byte_t buf[clen];
...
int main(int argc, char **argv) {
  ...
  byte_t id_hash[crypto_ghash_BYTES];
  crypto_generichash(id_hash, crypto_ghash_BYTES, rec_id, sizeof rec_id, NULL, 0);
  //printhex("hash for generating private key_state for bob", id_hash, crypto_ghash_BYTES);
  element_t id_elem;
  element_init(id_elem, ibe.pairing.G2);
  element_from_hash(id_elem, id_hash, crypto_ghash_BYTES);
  element_t d_id;
