...
int bloom_init(bloomfilter_s *bf, double p, uint32_t n, uint64_t hash_key, uint8_t *data, uint32_t prefix_len);
void bloom_clear(struct bloomfilter_s *bf);
void bloom_free(struct bloomfilter_s *bf);
void bloom_print_stats(bloomfilter_s *bf);
bloomfilter_s *bloom_alloc(double p, uint32_t n, uint64_t hash_key, uint8_t *data, uint32_t prefix_len);
