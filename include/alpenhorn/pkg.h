...
struct pkg
{
    ...
    // key_state + signature
    uint8_t eph_broadcast_message[header_BYTES + pkg_broadcast_msg_BYTES];
...
void pkg_client_init(pkg_client* client, pkg_s* server, const uint8_t* user_id, const uint8_t* lt_sig_key);

void pkg_new_ibe_keypair(pkg_s* server);

int pkg_server_init(pkg_s* server, uint64_t id, uint64_t num_clients, int num_threads, char* user_data_path);

void pkg_extract_client_sk(pkg_s* server, pkg_client* client);

void pkg_sign_for_client(pkg_s* server, pkg_client* client);

void pkg_encrypt_client_response(pkg_s* server, pkg_client* client);

void pkg_client_free(pkg_client* client);

void pkg_new_round(pkg_s* server);

int pkg_auth_client(pkg_s* server, pkg_client* client, uint8_t* auth_msg_buf);

int pkg_client_lookup(pkg_s* server, uint8_t* user_id);

int pkg_parallel_operation(pkg_s* server, void* (* operator)(void*), uint8_t* data_ptr, uint64_t data_elem_length);

int pkg_registration_request(pkg_s* server, const uint8_t* user_id, uint8_t* sig_key);

int pkg_confirm_registration(pkg_s* server, uint8_t* user_id, uint8_t* sig);

int pkg_server_startup(pkg_s* pkg);

void pkg_server_run(pkg_s* s);

void pkg_server_shutdown(pkg_s* server);
