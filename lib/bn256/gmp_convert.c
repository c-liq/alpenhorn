...
void fp2mpz2(mpz_t rop, const fpe_t op)
{
	mpz_t v, vp, t, p;
	mpz_init(t);

	mpz_init_set_ui(v, (unsigned long) bn_v);
	mpz_init_set_ui(vp, (unsigned long) bn_v);
	...
	int i = 2;
	for (int factor = 6; factor <= 36; factor *= 6) {
		for (int j = 0; j < 5; j++) {
			mpz_mul(vp, vp, v);
			mpz_set_si(t, ftoll(op->v[i]));
			mpz_mul_si(t, t, factor);
			mpz_mul(t, t, vp);
			mpz_add(rop, rop, t);
			i++;
		}
...
void fp2mpz(mpz_t rop, const fpe_t op)
{
	...
	mpz_clear(p);
	mpz_clear(t);
	//gmp_printf("MPZ INT SERIALIZED: %Zd\n", rop);
	mpz_clear(vp);
}
...
void mpz2fp2(fpe_t rop, const mpz_t op)
{
	...
	mpz_mul_si(vp, v, 6);
	mpz_tdiv_qr(acc, tmp, acc, vp);
	rop->v[0] = mpz_get_d(tmp);

	for (int i = 1; i < 6; i++) {
		mpz_tdiv_qr(acc, tmp, acc, v);
		rop->v[i] = mpz_get_d(tmp);
	}

	mpz_tdiv_qr(acc, tmp, acc, vp);

	rop->v[6] = mpz_get_d(tmp);
...
//	gmp_printf("MPZ INT DESERIALIZED: %Zd\n", op);
	...
	rop->v[0] = mpz_get_d(r);
	mpz_tdiv_qr_ui(t, r, t, (unsigned long) bn_v);
	rop->v[1] = mpz_get_d(r);
	mpz_tdiv_qr_ui(t, r, t, (unsigned long) bn_v);
	rop->v[2] = mpz_get_d(r);
	mpz_tdiv_qr_ui(t, r, t, (unsigned long) bn_v);
