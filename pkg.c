...
void pkg_client_init(pkg_client *client, pkg_server *server, byte_t *user_id, byte_t *lt_sig_key) {
  ...
  sodium_hex2bin(client->long_term_sig_pub_key,
                 ...
                 NULL,
                 NULL);
  memcpy(client->round_signature_message + dialling_round_length, client->user_id, af_email_string_bytes);
  memcpy(client->round_signature_message + dialling_round_length + af_email_string_bytes,
  ...
  crypto_generichash(id_hash, crypto_generichash_BYTES, client->user_id, af_email_string_bytes, NULL, 0);
...
void pkg_sign_for_client(pkg_server *server, pkg_client *client) {
  ...
  printhex("srv signing msg", client->round_signature_message, round_sig_message_length);
