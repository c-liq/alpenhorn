...
int client_register(client_s *c)
{
	if (!c || c->registered) {
		fprintf(
			stderr,
			"client trying to register is already associated with an account\n");
		return -1;
	}
	int res = crypto_sign_keypair(c->lt_sig_keypair.public_key,
	...
	serialize_uint32(c->register_buf + net_msg_type_BYTES,
	                 cli_pkg_reg_request_BYTES);
	memcpy(c->register_buf + net_header_BYTES, c->user_id, user_id_BYTES);
	memcpy(c->register_buf + net_header_BYTES, c->lt_sig_keypair.public_key,
...
int client_reg2(sign_keypair *sig_keys, char *user_id)
{
	if (!sig_keys || !user_id) return -1;
	...
	serialize_uint32(register_buf, CLIENT_REG_REQUEST);
	serialize_uint32(register_buf + net_msg_type_BYTES, cli_pkg_reg_request_BYTES);
	memcpy(register_buf + net_header_BYTES, user_id, user_id_BYTES);
	memcpy(register_buf + net_header_BYTES, sig_keys->public_key, crypto_sign_PUBLICKEYBYTES);
...
int client_confirm_registration(uint8_t *user_id, sign_keypair *sig_keys, char *msgs_buf)
{
	uint8_t reg_confirm_msg[net_header_BYTES + user_id_BYTES + crypto_sign_BYTES];
	memset(reg_confirm_msg, 0, sizeof reg_confirm_msg);
	serialize_uint32(reg_confirm_msg, CLIENT_REG_CONFIRM);
	memcpy(reg_confirm_msg + net_header_BYTES, user_id, user_id_BYTES);
	...
	for (uint32_t i = 0; i < num_pkg_servers; i++) {
		uint8_t nonce_bytes[crypto_ghash_BYTES];
		memset(nonce_bytes, 0, sizeof nonce_bytes);
		sodium_hex2bin(nonce_bytes,
		               crypto_ghash_BYTES,
		...
		crypto_sign(reg_confirm_msg + sig_offset, NULL, nonce_bytes, crypto_ghash_BYTES, sig_keys->secret_key);

		int socket_fd = net_connect("127.0.0.1", pkg_cl_listen_ports[i], 0);
		if (socket_fd == -1) {
			fprintf(stderr, "pkg server connection failure\n");
			return -1;
		}

		int res = net_send_blocking(socket_fd, reg_confirm_msg, sizeof reg_confirm_msg);
		if (res) {
			fprintf(stderr, "socket send failure\n");
			return -1;
		}

		uint8_t response_buf[net_header_BYTES];
		res = net_read_blocking(socket_fd, response_buf, net_header_BYTES);
...
int dial_call_friend(client_s *c,
                     ...
                     const uint32_t intent)
{
	...
	result = kw_dialling_token(c->dial_request_buf + net_header_BYTES + mb_BYTES,
...
int af_add_friend(client_s *c, const char *user_id)
{
	...
	memset(c->friend_request_id, 0, sizeof c->friend_request_id);
...
int af_confirm_friend(client_s *c, const char *user_id)
{
	...
	int result = af_accept_request(c, req);
	if (result)
		return -1;
...
int af_fake_request(client_s *c)
{
	...
	// To avoid distributing cover requests, set the mailbox to an invalid number
	// so last mix server can discard them
	...
	serialize_uint32(c->friend_request_buf + net_header_BYTES,
...
int dial_fake_request(client_s *c)
{
	...
	serialize_uint32(c->dial_request_buf, CLIENT_DIAL_MSG);
	serialize_uint32(c->dial_request_buf + net_msg_type_BYTES,
...
int dial_process_mb(client_s *c,
                    uint8_t *mb_data,
                    uint64_t round,
                    uint32_t num_tokens)
{
...
int af_create_pkg_auth_request(client_s* c) {
  ...
  for (int i = 0; i < num_pkg_servers; i++) {
	...
	curvepoint_fp_add_vartime(c->pkg_eph_pub_combined_g1,
							  c->pkg_eph_pub_combined_g1, g1_tmp);
  ...
  // bn256_deserialize_and_sum_g1(c->pkg_eph_pub_combined_g1,
  // c->pkg_broadcast_msgs[0], num_pkg_servers);
  return 0;
}
...
int onion_encrypt_message(client_s *c,
                          uint8_t *msg,
                          uint32_t base_msg_length,
                          bool is_dial)
{
	if (!c || !msg)
		return -1;
...
int add_onion_encryption_layer(client_s *client,
                               uint8_t *msg,
                               uint32_t base_msg_len,
                               uint32_t srv_id,
                               bool is_dial)
{
	if (!client || !msg)
                       ...
                       const sign_keypair *signing_keys)
{
	...
	int result = client_init(client, user_id, signing_keys);
	if (result) {
		free(client);
		return NULL;
	}
...
int net_send_message(client_s *s,
                     connection *conn,
                     uint8_t *msg,
...
void client_update_af_keys(client_s *client, uint8_t *buffer)
{
	...
	for (int i = 0; i < num_mix_servers; i++) {
		...
		//	printhex("new af dh key", client->mix_af_pks[i], crypto_pk_BYTES);
		ptr += crypto_pk_BYTES;
	}
}
...
int client_net_pkg_register(client_s *cn)
{
	...
	for (int i = 0; i < num_pkg_servers; i++) {
		connection *conn = &cn->net_state.pkg_connections[i];
		int res = net_send_message(cn, conn, cn->register_buf,
		                           net_header_BYTES + cli_pkg_reg_request_BYTES);
		if (res)
			return -1;
	}
	return 0;
...
int mix_last_process_msg(void *client_ptr, connection *conn)
{
	...
	case NEW_AFMB_AVAIL:
		serialize_uint32(
			conn->write_buf.data + conn->bytes_written + conn->write_remaining,
			CLIENT_AF_MB_REQUEST);
		serialize_uint32(conn->write_buf.data + conn->bytes_written +
			                 conn->write_remaining + 4,
		                 user_id_BYTES);
		memcpy(conn->write_buf.data + conn->bytes_written +
			       conn->write_remaining + 8,
		       conn->read_buf.data + 8, round_BYTES);
		memcpy(conn->write_buf.data + conn->bytes_written +
			       conn->write_remaining + net_header_BYTES,
		       client->user_id, user_id_BYTES);
...
int client_run(client_s *client)
{
	...
	client->dialling_round =
		deserialize_uint64(net_state->mix_entry.read_buf.data + 16);
	...
	while (client->running) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (events[i].events & EPOLLERR || events[i].events & EPOLLHUP) {
				...
				printf("Read on interrupt fd detected\n");
				continue;
			}
