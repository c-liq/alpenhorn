...
int client_register(sign_keypair *sig_keys, char *user_id)
{
	...
	serialize_uint32(register_buf + net_msg_type_BYTES,
                                ...
                                uint8_t *msgs_buf)
{
	...
	serialize_uint32(reg_confirm_msg + net_msg_type_BYTES,
	                 net_msg_size_bytes - net_header_BYTES);
	...
	for (uint32_t i = 0; i < num_pkg_servers; i++) {
		...
		/*if (response != PKG_CONFIRM_CLIENT_REG) {
			fprintf(stderr, "pkg failed to confirm registration | %u\n",
	response);
			return -1;
	}*/
	}
...
struct outgoing_friend_request
{
	uint64_t dialing_round;
	uint8_t user_id[user_id_BYTES];
	uint8_t dh_pk[crypto_box_PUBLICKEYBYTES];
	uint8_t client_sign_pk[crypto_sign_PUBLICKEYBYTES];
	uint8_t client_signature[crypto_sign_BYTES];
	uint8_t pkg_multisig[g1_serialized_bytes];
};
...
int af_onion_encrypt_request(client_s *client)
{
	...
	return onion_encrypt_message(client, client->friend_request_buf + net_header_BYTES, af_ibeenc_request_BYTES, false);
}
