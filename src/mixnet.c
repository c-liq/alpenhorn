...
void mix_af_distribute(mix_s *mix)
{
	...
	for (uint32_t i = 0; i < c->num_mailboxes; i++) {
		...
		net_serialize_header(mb->data, AF_MB, mailbox_sz - net_header_BYTES,
...
int mix_update_mailbox_counts(uint32_t n,
...
void mix_dial_decrypt_messages(mix_s *mix)
{
	...
	serialize_uint32(mix->dial_data.out_buf.data + net_msg_type_BYTES,
...
void mix_af_decrypt_messages(mix_s *mix)
{
	...
	serialize_uint32(mix->af_data.out_buf.data + net_msg_type_BYTES,
	                 mix->af_data.num_out_msgs * mix->af_data.out_msg_length);
}
...
int mix_exit_process_client_msg(void *owner, connection *conn)
{
	...
	if (conn->msg_type == CLIENT_DIAL_MB_REQUEST) {
		...
		if (request_mb) {
			net_epoll_queue_write(mix, conn, request_mb->bloom.base_ptr,
...
void mix_exit_new_af_key(mix_s *mix)
{
	...
	serialize_uint32(net_state->af_client_broadcast.data + net_msg_type_BYTES,
	                 crypto_pk_BYTES);
	net_epoll_queue_write(mix, &net_state->prev_mix,
...
int mix_process_mix_msg(void *m, connection *conn)
{
	...
	if (conn->msg_type == NEW_AF_ROUND) {
		...
		if (mix->server_id > 0) {
			net_epoll_queue_write(mix, &net_state->prev_mix, dial_broadcast_buf->data,
		...
		if (mix->is_last) {
			...
			printf(
				"Dial Round %lu: Received %d msgs, added %u noisem, discarded %u "
...
{
	mix_s *mix = (mix_s *) s;
	net_epoll_queue_write(s, conn, mix->net_state.bc_buf.data, net_header_BYTES,
	                      0);
}
...
void mix_broadcast_new_afr(mix_s *s)
{
	...
	while (conn) {
		net_epoll_queue_write(s, conn, af_broadcast->data, af_broadcast->capacity,
		                      0);
		conn = conn->next;
	}
}
...
int mix_entry_sync(mix_s *mix)
{
	...
	serialize_uint32(net_state->dial_client_broadcast.data, NEW_DIAL_ROUND);
	serialize_uint32(net_state->af_client_broadcast.data + net_msg_type_BYTES,
	                 net_state->af_client_broadcast.capacity - net_header_BYTES);
	serialize_uint32(
		net_state->dial_client_broadcast.data + net_msg_type_BYTES,
		net_state->dial_client_broadcast.capacity - net_header_BYTES);
...
{
	net_epoll_queue_write(mix, &mix->net_state.next_mix, buf->data, buf->used,
	                      NULL);
}
...
void net_epoll_send_queue(mix_s *s, connection *conn)
{
	...
	if (conn->send_queue_head && !(conn->event.events & EPOLLOUT)) {
		...
		epoll_ctl(s->net_state.epoll_fd, EPOLL_CTL_MOD, conn->sock_fd,
		          &conn->event);
	}
