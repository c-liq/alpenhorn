...
int af_create_pkg_auth_request(client_s *c) {
  ...
  for (int i = 0; i < num_pkg_servers; i++) {
	...
	serialize_uint64(auth_request + net_header_BYTES, c->af_round + 1);
	...
	int res = crypto_sign_detached(client_sig, NULL, auth_request + net_header_BYTES,
...
int af_onion_encrypt_request(client_s *client) {
  ...
  uint8_t *message_ptr = client->friend_request_buf + net_header_BYTES;
...
int client_init(client_s *c,
  ...
  if (byte_buffer_init(&c->af_mb_buffer, 1024*64)) {
...
int mix_entry_process_msg(void *client_ptr, connection *conn) {
  ...
  case NEW_AF_ROUND: client->af_start_round = get_time();
	...
	net_send_message(client,
					 &client->net_state.mix_entry,
					 client->friend_request_buf,
					 net_header_BYTES + onionenc_friend_request_BYTES);
	break;
  case NEW_DIAL_ROUND: client->dial_start_round = get_time();
	...
	break;
  case MIX_SYNC: client->af_round = deserialize_uint64(conn->read_buf.data + (net_msg_len_BYTES + net_msg_type_BYTES));
...
int mix_last_process_msg(void *client_ptr, connection *conn) {
  ...
  case DIAL_MB:
	...
	break;
  case AF_MB: byte_buffer_clear(&client->af_mb_buffer);
	...
	//af_parallel_process_mb(client, client->af_mb_buffer.data, client->af_mb_num_messages, client->af_round);
