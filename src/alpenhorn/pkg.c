...
int pkg_registration_request(pkg *server, byte_buffer *buf) {
    ...
    bb_write(email_buffer, (u8 *) date, strlen(date));
    char to_string[1024];
    char body_string[1024];
    sprintf(to_string, "To: %s\r\n", user_id);
    bb_write(email_buffer, (u8 *) to_string, strlen(to_string));
    bb_write(email_buffer, (u8 *) from, strlen(from));
    bb_write(email_buffer, (u8 *) subject, strlen(subject));

    sodium_bin2hex(body_string, crypto_ghash_BYTES * 2 + 1, pc->confirmation_key, crypto_ghash_BYTES);
    bb_write(email_buffer, (u8 *) body_string, strlen(body_string));
    ...
    list_push_head(server->registration_requests, pc);

    return 0;
}

int pkg_compare_reg_requests(const void *iter, const void *lookup) {
    const pkg_pending_client *pa = iter;
    return memcmp(pa->sig_key, lookup, crypto_sign_PUBLICKEYBYTES);
}

pkg_pending_client *pkg_lookup_pending_reg(pkg *server, u8 *user_id) {
    return list_find(server->registration_requests, user_id, pkg_compare_reg_requests);
}
...
int pkg_confirm_registration(pkg *server, byte_buffer *buf) {
    ...
    list_remove(server->registration_requests, pc, NULL);
    return 0;
}

static int pkg_load_user_data(pkg *pkg, u64 num_clients, const char *user_data_path);
...
int pkg_server_init(pkg *pkg, u64 server_id, u64 num_users, int num_threads, char *user_data_path) {
    ...
    if (user_data_path) {
        pkg_load_user_data(pkg, num_users, user_data_path);
    }

    pkg->registration_requests = NULL;
...
static int pkg_load_user_data(struct pkg *pkg, u64 num_clients, const char *user_data_path) {
    ...
    u8 *client_data_buffer = calloc(1, data_size);
...
void thpool_auth_client(void *arg) {
    ...
    pkg_client *client = conn->client;
    byte_buffer *buf = args->buf;

    if (!client) {
        pkg *srv = args->pkg_s;
        u8 *user_id = buf->read_pos + round_BYTES;
        assert(user_id);
        ...
        conn->client = &srv->clients[index];
        client = conn->client;
    }

    if (pkg_auth_client(client->server, client, conn, buf)) {
...
void
pkg_client_init(pkg_client *client, pkg *server, const u8 *user_id, const u8 *lt_sig_key) {
    client->server = server;
    serialize_u64(client->auth_data, PKG_AUTH_RES_MSG);
...
void pkg_broadcast_round(pkg *pkg) {
    ...
    for (list_item *hd = pkg->net_state.clients->head; hd; hd = hd->next) {
...
int pkg_auth_client(pkg *server, pkg_client *client, connection *conn, byte_buffer *buf) {
    ...
    crypto_salsa_encrypt(ctext_ptr, client->auth_data, pkg_auth_res_BYTES, symmetric_key);
...
void pkg_new_ibe_keypair(pkg *server) {
    bn256_g1_random(server->ibe_master_pk, server->ibe_master_sk);
...
void pkg_gen_identity_sk(pkg *server, pkg_client *client) {
    ...
    twistpoint_fp2_scalarmult_vartime(client_sk, client->hashed_id, server->ibe_master_sk);
    twistpoint_fp2_makeaffine(client_sk);
    bn256_serialize_g2(client->auth_data + bn256_bls_sig_message_bytes, client_sk);
}

void pkg_gen_certificate(pkg *pkg, pkg_client *client) {
    serialize_u64(client->certificate_msg, pkg->round);
    bn256_bls_sign_message(client->auth_data, client->certificate_msg, pkg_sig_message_BYTES, pkg->sig_keys.sk);
}
...
int pkg_net_process_client_msg(void *srv, connection *conn) {
    ...
    switch (header->type) {
        ...
        case CLIENT_REG_REQUEST:
            pkg_registration_request(s, buf);
            break;
        case CLIENT_REG_CONFIRM:
            pkg_confirm_registration(s, buf);
            break;
        default:
            fprintf(stderr, "Invalid message type from client\n");
    }
    return 0;
}

int pkg_server_startup(pkg *pkg) {
    net_server *s = &pkg->net_state;
    s->owner = pkg;
    s->clients = NULL;
    s->epoll_fd = epoll_create1(0);

    net_connect_init(&pkg->mix_conn, mix_server_ip, mix_server_port, s->epoll_fd, 1, 512, pkg_mix_read);

    s->listen_conn.sock_fd = net_listen_socket(pkg_cl_listen_ports[pkg->id], 1);
...
void pkg_server_run(pkg *s) {
    net_server *net_state = &s->net_state;
    net_state->events = calloc(epoll_num_events, sizeof *net_state->events);
    struct epoll_event *events = net_state->events;

    for (;;) {
        int n = epoll_wait(net_state->epoll_fd, events, epoll_num_events, 5000);
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (events[i].events & EPOLLERR || events[i].events & EPOLLHUP) {
                ...
                int res = net_accept_client(net_state, NULL, pkg_net_process_client_msg);
                if (res) {
