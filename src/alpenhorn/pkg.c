                         ...
                         uint8_t *sig_key)
{
    ...
    byte_buffer_s *email_buffer = bb_alloc(1024, false);
                ...
                char *user_data_path)
{
    ...
    if (user_data_path) {
        ...
        u64 data_size = num_clients * (user_id_BYTES + crypto_sign_PUBLICKEYBYTES + crypto_sign_SECRETKEYBYTES);
        uint8_t *client_data_buffer = calloc(1, data_size);
        if (!user_file) {
            fprintf(stderr, "failed to open user data file, terminating\n");
            exit(EXIT_FAILURE);
        }
        u64 x = fread(client_data_buffer,
...
void thpool_auth_client(void *arg)
{
    ...
    if (!authed) {
        ...
        bb_write(&conn->write_buf, client->eph_client_data, header_BYTES + pkg_enc_auth_res_BYTES);
...
pkg_client_parallel_init(void *args)
{
    ...
    for (int i = th_args->begin; i < th_args->end; i++) {
        //pkg_extract_client_sk(srv, &srv->clients[i]);
        //pkg_sign_for_client(srv, &srv->clients[i]);
        pkg_client_init(&srv->clients[i], srv, data, data + user_id_BYTES);
...
#if USE_PBC
void
pkg_new_ibe_keypair(pkg* server)
{
...
void
pkg_extract_client_sk(pkg* server, pkg_client* client)
{
...
void
pkg_sign_for_client(pkg* server, pkg_client* client)
{
...
bool pkg_net_auth_client(pkg_s *s, connection *conn)
{
    ...
    if (!authed) {
        ...
        bb_write(&conn->write_buf, client_state->eph_client_data, header_BYTES + pkg_enc_auth_res_BYTES);
        net_epoll_send(conn, conn->sock_fd);
    }
    return true;
}
...
pkg_net_process_client_msg(void *srv, connection *conn, byte_buffer_s *buf)
{
    ...
    else if (header->type == CLIENT_REG_REQUEST) {
        pkg_registration_request(s,
                                 conn->read_buf.data + header_BYTES,
                                 conn->read_buf.data + header_BYTES +
                                     user_id_BYTES);

        alp_serialize_header(&conn->write_buf, PKG_REG_REQUEST_RECEIVED, 0, 0, 0);
        net_epoll_send(conn, conn->sock_fd);
    }

    else if (header->type == CLIENT_REG_CONFIRM) {
...
pkg_server_startup(pkg_s *pkg)
{
    ...
    connection_init(&pkg->mix_conn, read_buf_SIZE, write_buf_SIZE, NULL, s->epoll_fd, mix_fd);

    s->listen_conn.sock_fd = net_start_listen_socket(pkg_cl_listen_ports[pkg->srv_id], 1);
    ...
    for (;;) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (events[i].events & EPOLLERR || events[i].events & EPOLLHUP) {
                ...
                int res = net_epoll_client_accept(net_state, NULL, NULL);
                if (res) {
