...
void mix_af_distribute(mix_s *mix)
{
	...
	for (uint64_t i = 0; i < c->num_mailboxes; i++) {
		...
		net_serialize_header(mb->data, AF_MB, (uint64_t) mailbox_sz - net_header_BYTES,
	...
	for (uint64_t i = 0; i < mix->af_data.num_out_msgs; i++) {
...
{
	uint64_t i = mix->dial_cont_stack_head;
...
void mix_dial_distribute(mix_s *mix)
{
	...
	for (uint64_t i = 0; i < c->num_mailboxes; i++) {
		...
		net_serialize_header(mb->bloom.base_ptr, DIAL_MB, (uint64_t) mb->bloom.size_bytes,
		                     c->round, mb->num_messages);
	}
...
int mix_init(mix_s *mix, uint64_t server_id, uint64_t num_threads, uint64_t num_servers)
{
	...
	sprintf(log_file_str, "mix_%ld.log", server_id);
	...
	uint64_t inc_onion_layer_bytes = (mix->num_inc_onion_layers * onion_layer_BYTES);
                          ...
                          uint64_t msg_length)
{
	...
	for (uint64_t i = msg_count - 1; i >= 1; i--) {
		uint64_t j = randombytes_uniform(i);
...
/*	double start_timer = get_time();
	...
	LOG_OUT(mix->log_file,
	        "Time taken to shuffle %ld AF msgs: %f\n",
	        mix->af_data.num_out_msgs,
...
void mix_dial_shuffle(mix_s *mix)
{
	...
	LOG_OUT(mix->log_file,
	        "Time taken to shuffle %ld Dial msgs: %f\n",
	        mix->dial_data.num_out_msgs,
	        get_time() - start_timer);
}
                        ...
                        uint8_t *matching_pub_dh)
{
	...
	uint64_t message_length = msg_len + (onion_layer_BYTES * index);
                          ...
                          uint8_t **keys)
{
	...
	for (uint64_t i = 0; i < mix->num_out_onion_layers; i++) {
...
void mix_dial_add_noise(mix_s *mix)
{
	...
	uint64_t num_mailboxes = mix->dial_data.num_mailboxes;
	for (uint64_t i = 0; i < num_mailboxes; i++) {
		uint64_t noise = laplace_rand(&mix->dial_data.laplace);
...
void *mix_af_mailbox_noise(void *mbid)
{
	#if !USE_PBC
	scalar_t random;
	curvepoint_fp_t tmp;
	...
	struct mix_mailbox_noise *mb_noise = mbid;
	mix_s *mix = mb_noise->mix;
	uint64_t mb_id = mb_noise->mailbox_id;
	uint64_t noise = laplace_rand(&mix->af_data.laplace);
	uint8_t *buf = calloc(noise, mix->af_data.out_msg_length);
	uint8_t *curr_ptr = buf;
	for (int j = 0; j < noise; j++) {
		serialize_uint64(curr_ptr, mb_id);
#if USE_PBC
		...
		mix_onion_encrypt_msg(mix, curr_ptr, af_ibeenc_request_BYTES + mb_BYTES,
		                      mix->mix_af_dh_pks);
		curr_ptr += mix->af_data.out_msg_length;
	}
	pthread_mutex_lock(mb_noise->mutex);
	byte_buffer_put(&mix->af_data.out_buf, buf, noise * mix->af_data.out_msg_length);
	mix->af_data.num_out_msgs += noise;
	mix->af_data.last_noise_count += noise;
	if (mix->num_out_onion_layers == 0) {
		mix->af_data.mb_counts[mb_id] += noise;
	}
	pthread_mutex_unlock(mb_noise->mutex);
	free(buf);
}

void mix_af_add_noise_parallel(mix_s *mix)
{
	pthread_mutex_t *mut = calloc(1, sizeof *mut);
	pthread_mutex_init(mut, NULL);
	double start_timer = get_time();
	uint64_t num_threads = mix->af_data.num_mailboxes + 1;
	pthread_t threads[num_threads];
	struct mix_mailbox_noise args[num_threads];
	...
	for (uint64_t i = 0; i < num_mailboxes; i++) {
		...
		args[i].mix = mix;
		args[i].mutex = mut;
		pthread_create(&threads[i], NULL, mix_af_mailbox_noise, &args[i]);
	}
                                ...
                                uint8_t *sk)
{
	...
	uint64_t ctextlen = onionm_len - (crypto_pk_BYTES + crypto_NBYTES);
...
mix_dial_parallel_dispatch(mix_s *server)
{
	...
	uint64_t num_per_thread = server->dial_data.num_inc_msgs / num_threads;
	uint64_t leftover_msgs = server->dial_data.num_inc_msgs;
	int curindex = 0;
	...
	for (int i = 0; i < num_threads - 1; i++) {
		...
		//printf("Thread %ld taking messages %ld to", i, curindex);
	...
	LOG_OUT(server->log_file,
	        "Time taken to decrypt %ld Dial msgs: %f\n",
	        server->dial_data.num_inc_msgs,
	        get_time() - start_timer);
...
mix_af_parallel_dispatch(mix_s *server)
{
	...
	pthread_t threads[num_threads];
	mix_thread_args args[num_threads];
	uint64_t num_per_thread = server->af_data.num_inc_msgs / num_threads;
	uint64_t leftover_msgs = server->af_data.num_inc_msgs;
	int curindex = 0;
	...
	LOG_OUT(server->log_file,
	        "Time taken to decrypt %ld AF msgs: %f\n",
	        server->af_data.num_inc_msgs,
	        get_time() - start_timer);
	return 0;
}
...
void mix_pkg_broadcast(mix_s *s, uint64_t msg_type)
{
	uint8_t buf[net_header_BYTES];
	memset(buf, 0, net_header_BYTES);
	serialize_uint64(buf, msg_type);
	serialize_uint64(buf + (net_msg_len_BYTES + net_msg_type_BYTES), s->af_data.round);
...
int mix_exit_process_client_msg(void *owner, connection *conn)
{
	...
	if (conn->msg_type == CLIENT_DIAL_MB_REQUEST) {
		uint64_t mb_round = deserialize_uint64(conn->read_buf.data + (net_msg_len_BYTES + net_msg_type_BYTES));
		...
		if (request_mb) {
			net_epoll_queue_write(&mix->net_state, conn, request_mb->bloom.base_ptr,
			                      request_mb->bloom.total_size_bytes, true);
		}
	}
	else if (conn->msg_type == CLIENT_AF_MB_REQUEST) {
		//uint64_t round_num = deserialize_uint64(conn->read_buf.data + (net_msg_len_BYTES + net_msg_type_BYTES));
		...
		net_epoll_queue_write(&mix->net_state, conn, mailbox->data, mailbox->size_bytes, true);
	}
...
void mix_exit_new_af_key(mix_s *mix)
{
	...
	serialize_uint64(net_state->af_client_broadcast.data, NEW_AF_ROUND);
	serialize_uint64(net_state->af_client_broadcast.data + net_msg_type_BYTES,
	                 crypto_pk_BYTES);
...
void mix_exit_new_dial_key(mix_s *mix)
{
	...
	serialize_uint64(net_state->dial_client_broadcast.data, NEW_DIAL_ROUND);
	serialize_uint64(net_state->dial_client_broadcast.data + net_msg_type_BYTES,
	                 crypto_pk_BYTES);
...
int mix_process_mix_msg(void *m, connection *conn)
{
	...
	if (conn->msg_type == NEW_AF_ROUND) {
		...
		serialize_uint64(net_state->af_client_broadcast.data + (net_msg_len_BYTES + net_msg_type_BYTES),
		                 mix->af_data.round + 1);
		byte_buffer_put_virtual(&net_state->af_client_broadcast, net_header_BYTES);
		byte_buffer_put(&net_state->af_client_broadcast, mix->mix_af_dh_pks[0],
		                crypto_pk_BYTES);
		...
		serialize_uint64(dial_broadcast_buf->data, NEW_DIAL_ROUND);
		serialize_uint64(dial_broadcast_buf->data + net_header_BYTES, mix->dial_data.round + 1);
		serialize_uint64(dial_broadcast_buf->data + net_msg_type_BYTES,
		                 conn->curr_msg_len + crypto_pk_BYTES);
		...
		if (mix->is_last) {
			...
			LOG_OUT(stdout,
			        "AF Round %lu: Received %ld msgs, added %ld noise, discarded %ld "
				        "cover msgs -> Distributing %ld\n",
			...
			mix->af_data.num_mailboxes = deserialize_uint64(conn->read_buf.data + 24);
			mix_af_newround(mix);
		}
		else {
			mix_af_s *af = &mix->af_data;
			LOG_OUT(stdout,
			        "AF Round %lu: Received %ld msgs, added %ld noise, forwarding %ld\n",
			        af->round, af->num_inc_msgs, af->last_noise_count, af->num_out_msgs);
			mix->af_data.num_mailboxes = deserialize_uint64(conn->read_buf.data + net_header_BYTES);
		...
		if (mix->is_last) {
			...
			uint64_t discarded = dd->num_inc_msgs + dd->last_noise_count - dd->num_out_msgs;
			LOG_OUT(stdout,
			        "Dial Round %lu: Received %ld msgs, added %lunoise, discarded %lu"
				        "noise -> Distributing %ld\n",
			...
			mix->dial_data.num_mailboxes = deserialize_uint64(conn->read_buf.data + 24);
			mix_dial_newround(mix);
		}
		else {
			mix_dial_s *dd = &mix->dial_data;
			LOG_OUT(stdout,
			        "Dial Round %lu: Received %ld msgs, added %lunoise -> forwarding %ld\n",
			        dd->round, dd->num_inc_msgs, dd->last_noise_count,
			        dd->num_out_msgs);
			byte_buffer_s *buf = &mix->dial_data.out_buf;
			mix->dial_data.num_mailboxes = deserialize_uint64(conn->read_buf.data + net_header_BYTES);
...
int mix_connect_neighbour(uint64_t srv_id)
{
	if (srv_id <= 0) {
...
{
	serialize_uint64(s->net_state.bc_buf.data + 24, s->dial_data.round);
	connection *conn = s->net_state.clients;
	byte_buffer_s *dial_broadcast = &s->net_state.dial_client_broadcast;
	serialize_uint64(dial_broadcast->data + (net_msg_len_BYTES + net_msg_type_BYTES), s->dial_data.round);
	serialize_uint64(dial_broadcast->data + 24, s->dial_data.num_mailboxes);
...
void mix_broadcast_new_dialmb(mix_s *s, uint64_t round)
{
	...
	serialize_uint64(bc_buff, NEW_DMB_AVAIL);

	serialize_uint64(bc_buff + net_msg_type_BYTES, 0);
	serialize_uint64(bc_buff + net_msg_type_BYTES + net_msg_len_BYTES, round);
	connection *conn = s->net_state.clients;
	while (conn) {
...
void mix_broadcast_new_afmb(mix_s *s, uint64_t round)
{
	...
	serialize_uint64(bc_buff, NEW_AFMB_AVAIL);
	serialize_uint64(bc_buff + net_msg_type_BYTES, 0);
	serialize_uint64(bc_buff + net_msg_type_BYTES + net_msg_len_BYTES, round);
	connection *conn = s->net_state.clients;
	while (conn) {
		memcpy(conn->write_buf.data + conn->bytes_written + conn->write_remaining, bc_buff, net_header_BYTES);
...
void mix_broadcast_new_afr(mix_s *s)
{
	...
	serialize_uint64(af_broadcast->data + 24, s->af_data.num_mailboxes);
	while (conn) {
...
int mix_entry_sync(mix_s *mix)
{
	...
	serialize_uint64(bc_buf->data, MIX_SYNC);
	serialize_uint64(bc_buf->data + net_msg_type_BYTES, 0);
	serialize_uint64(bc_buf->data + (net_msg_len_BYTES + net_msg_type_BYTES), mix->af_data.round);
	serialize_uint64(bc_buf->data + 24, mix->dial_data.round);
	...
	serialize_uint64(net_state->dial_client_broadcast.data, NEW_DIAL_ROUND);
	serialize_uint64(net_state->af_client_broadcast.data + net_msg_type_BYTES,
	                 (uint64_t) net_state->af_client_broadcast.capacity - net_header_BYTES);
	serialize_uint64(
		net_state->dial_client_broadcast.data + net_msg_type_BYTES,
		(uint64_t) net_state->dial_client_broadcast.capacity - net_header_BYTES);
	serialize_uint64(net_state->af_client_broadcast.data + (net_msg_len_BYTES + net_msg_type_BYTES),
	                 mix->af_data.round);
	serialize_uint64(net_state->dial_client_broadcast.data + net_header_BYTES,
	                 mix->dial_data.round);
	byte_buffer_put_virtual(&net_state->af_client_broadcast, net_header_BYTES);
	byte_buffer_put_virtual(&net_state->dial_client_broadcast, net_header_BYTES);
...
int mix_net_init(mix_s *mix)
{
	...
	uint64_t buffer_size = crypto_pk_BYTES * 2 * (num_mix_servers - mix->server_id);
	...
	byte_buffer_init(&net_state->dial_client_broadcast,
	                 net_header_BYTES + buffer_size / 2);
	serialize_uint64(net_state->bc_buf.data, MIX_SYNC);
	serialize_uint64(net_state->bc_buf.data + net_msg_type_BYTES, buffer_size);
	serialize_uint64(net_state->bc_buf.data + (net_msg_len_BYTES + net_msg_type_BYTES), mix->af_data.round);
	serialize_uint64(net_state->bc_buf.data + net_header_BYTES, mix->dial_data.round);
...
{
	serialize_uint64(mix->net_state.bc_buf.data + (net_msg_len_BYTES + net_msg_type_BYTES), mix->af_data.round);
...
void mix_entry_process_af_batch(mix_s *mix)
{
	...
	char time_buffer[40];
	get_current_time(time_buffer);
	LOG_OUT(stdout, "[Entry] AF Round %ld: Received %ld msgs, added %ld noise -> Forwarding %ld at %s\n",
	        af_data->round, af_data->num_inc_msgs, af_data->last_noise_count,
	        af_data->num_out_msgs, time_buffer);
	LOG_OUT(mix->log_file, "[Entry] AF Round %ld: Received %ld msgs, added %ld noise -> Forwarding %ld at %s\n",
	        af_data->round, af_data->num_inc_msgs, af_data->last_noise_count,
	        af_data->num_out_msgs, time_buffer);
	mix_af_decrypt_messages(mix);
	byte_buffer_clear(&mix->af_data.in_buf);
...
void mix_entry_process_dial_batch(mix_s *mix)
{
	...
	char time_buffer[40];
	get_current_time(time_buffer);
	LOG_OUT(stdout, "Dial Round %ld: Received %ld msgs, added %lunoise -> Forwarding %ld at %s\n",
	        dd->round, dd->num_inc_msgs, dd->last_noise_count, dd->num_out_msgs, time_buffer);
	LOG_OUT(mix->log_file, "Dial Round %ld: Received %ld msgs, added %lunoise -> Forwarding %ld at %s\n",
	        dd->round, dd->num_inc_msgs, dd->last_noise_count, dd->num_out_msgs, time_buffer);
	mix_dial_decrypt_messages(mix);
	mix_dial_shuffle(mix);
	byte_buffer_clear(&mix->dial_data.in_buf);
...
sim_mix_parallel_fake_client_traffic(mix_s *server, int p)
{
	...
	uint64_t num_mailboxes;
	num_mailboxes = p == 0 ? server->af_data.num_mailboxes : server->dial_data.num_mailboxes;
	uint64_t num_real_msgs = ((1000 / 100) * 5 / num_mailboxes);
	uint64_t num_cover_msgs = ((10000 / 100) * 95);

	uint64_t num_real_per_thread = num_real_msgs / num_threads;
	uint64_t num_cover_per_thread = num_cover_msgs / num_threads;
...
{
	mix_thread_args *targs = (mix_thread_args *) args;
	uint64_t num_real_msgs = targs->num_msgs;
	uint64_t num_cover_msgs = targs->num_fake_msgs;
	mix_s *mix = targs->mix;
	...
	uint8_t *curr_ptr = buf;
	for (uint64_t i = 0; i < num_mailboxes; i++) {
		uint64_t noise = num_real_msgs;
		if (i == num_mailboxes - 1) {
			noise = num_cover_msgs;
		}
		printf("Adding %ld msgs to mailbox %ld\n", noise, i);
		for (int j = 0; j < noise; j++) {
			serialize_uint64(curr_ptr, i);
			randombytes_buf(curr_ptr + sizeof i, dialling_token_BYTES);
...
void sim_mix_entry_dial_add_noise(void *args)
{
	...
	byte_buffer_put(&mix->dial_data.in_buf,
	                buf,
	                mix->dial_data.inc_msg_length * ((num_real_msgs * mix->dial_data.num_mailboxes) + num_cover_msgs));
	mix->dial_data.num_inc_msgs += (num_real_msgs * mix->dial_data.num_mailboxes) + num_cover_msgs;
	pthread_mutex_unlock(targs->mutex);
	free(buf);
}

void *mix_parallel_add_noise(void *args)
{
#if !USE_PBC
	scalar_t random;
	curvepoint_fp_t tmp;
	...
	mix_thread_args *targs = (mix_thread_args *) args;
	uint64_t num_noise_msgs = targs->num_msgs;

	mix_s *mix = targs->mix;

	uint8_t *buf = calloc(mix->af_data.out_msg_length, num_noise_msgs * mix->af_data.num_mailboxes);
	uint8_t *curr_ptr = buf;
	uint64_t num_mailboxes = mix->af_data.num_mailboxes + 1;
	for (uint64_t i = 0; i < num_mailboxes; i++) {
		uint64_t noise = num_noise_msgs;
	...
	byte_buffer_put(&mix->af_data.in_buf,
	                buf,
	                mix->af_data.inc_msg_length * (num_noise_msgs * mix->af_data.num_mailboxes));
	mix->af_data.num_inc_msgs += num_noise_msgs * mix->af_data.num_mailboxes;
	pthread_mutex_unlock(targs->mutex);
	free(buf);
}
...
void *sim_mix_af_entry_add_noise(void *args)
{
	...
	uint8_t *buf = calloc(mix->af_data.inc_msg_length, (num_noise_msgs * mix->af_data.num_mailboxes) + num_cover_msgs);
	uint8_t *curr_ptr = buf;
	uint64_t num_mailboxes = mix->af_data.num_mailboxes + 1;
	for (uint64_t i = 0; i < num_mailboxes; i++) {
		uint64_t noise = num_noise_msgs;
		...
		printf("Adding %ld msgs to AF mailbox %ld\n", noise, i);
		for (int j = 0; j < noise; j++) {
			serialize_uint64(curr_ptr, i);
			sim_mix_onion_encrypt_msg(mix, curr_ptr, af_ibeenc_request_BYTES + mb_BYTES, mix->mix_af_dh_pks);
			curr_ptr += mix->af_data.inc_msg_length;
	...
	byte_buffer_put(&mix->af_data.in_buf,
	                buf,
	                mix->af_data.inc_msg_length * ((num_noise_msgs * mix->af_data.num_mailboxes) + num_cover_msgs));
	mix->af_data.num_inc_msgs += (num_noise_msgs * mix->af_data.num_mailboxes) + num_cover_msgs;
	pthread_mutex_unlock(targs->mutex);
	free(buf);
}
...
int sim_mix_main(int argc, char **argv)
{
	...
	uint64_t num_threads = (uint64_t) strtol(argv[2], NULL, 10);
	...
	if (sid == 0) {
		...
		mix_s mix;
		mix_init(&mix, (uint64_t) sid, num_threads, 0);
		mix_net_init(&mix);
		mix_net_sync(&mix);
...
int mix_main(int argc, char **argv)
{
	...
	if (sid == 0) {
		...
		mix_s mix;
		mix_init(&mix, (uint64_t) sid, num_threads, 0);
		printf("mix %ld | inc length: %ld\n", mix.server_id, mix.dial_data.inc_msg_length);
		mix_net_init(&mix);
		mix_net_sync(&mix);
		mix_run(&mix, NULL, mix_exit_process_client_msg);
	}
